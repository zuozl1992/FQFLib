...
bool FQFDemux::openDemux(const char * path, OpenType openType)
{
    ...
    if(audioStream == -1 && videoStream == -1)
        mediaType = OtherMedia;
    else if(videoStream == -1)
    {
        if(openType == AudioFile)
            mediaType = AudioMedia_NoVideo;
        else if(openType == AudioStream)
    ...
    {
        if(openType == VideoFile)
            mediaType = VideoMedia_NoAudio;
        else if(openType == VideoStream)
        ...
        else if(openType == AudioFile)
        {
            videoStream = -1;
            mediaType = AudioMedia_NoVideo;
        }
        else if(openType == VideoStream || openType == AudioStream)
            mediaType = Stream;
        else
            mediaType = OtherMedia;
    }
...
int FQFDemux::getWidth()
{
    return width;
}

int FQFDemux::getHeight()
{
    return height;
}
